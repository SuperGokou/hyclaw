...
describe("titleForRoute", () => {
  // Titles are asserted in English; pin the locale so HYClaw's zh-CN default
  // does not turn these into Chinese strings.
  beforeAll(async () => {
...
describe("plugin tabs route", () => {
  ...
  it("keeps the plugins route defined but off the slim sidebar", () => {
    expect(SIDEBAR_NAV_ROUTES).not.toContain("plugin");
    expect(SIDEBAR_NAV_ROUTES).not.toContain("plugins");
    // Route still resolves (definition retained) even though nav hides it.
    expect(routeIdFromPath("/settings/plugins")).toBe("plugins");
    expect(routeIdFromPath("/plugins")).toBeNull();
  });
});
...
describe("SIDEBAR_NAV_ROUTES", () => {
  ...
  it("promotes skills to a direct sidebar entry (plugins hub removed)", () => {
    expect(SIDEBAR_NAV_ROUTES).toContain("skills");
    expect(SIDEBAR_NAV_ROUTES).not.toContain("skill-workshop");
    // HYClaw removed the plugins hub, so no route is a hub member anymore.
    expect(isPluginsHubRoute("plugins")).toBe(false);
    expect(isPluginsHubRoute("skills")).toBe(false);
    expect(isPluginsHubRoute("sessions")).toBe(false);
  });

  it("keeps the slim office settings slices routed but out of the sidebar", () => {
