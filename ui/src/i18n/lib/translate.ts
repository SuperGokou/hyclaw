...
class I18nManager {
  ...
  private resolveInitialLocale(): Locale {
    ...
    // HYClaw desktop shell injects a Chinese-first preference via a preload
    // global so the app defaults to zh-CN without changing library/test
    // behavior (tests have no such global and fall back to navigator).
    const injected = (globalThis as { __HYCLAW_LOCALE__?: unknown }).__HYCLAW_LOCALE__;
    if (isSupportedLocale(injected)) {
      return injected;
    }
    const language =
