...
import { fileToAvatarDataUrl } from "../agents/avatar-image.ts";
...
export class ProfilePage extends OpenClawLightDomElement {
  ...
  @state() private avatarUploading = false;
  ...
  /** Pick a local image, compress it to a bounded data URL, and save it as the
      featured agent's avatar so it shows in chat, sidebar, and here. */
  private async handleAvatarPick(agentId: string, event: Event) {
    ...
    const client = this.client;
    if (!client) {
      this.avatarError = t("profilePage.avatarNeedsConnection");
      return;
    }
    ...
    try {
      ...
      if (!dataUrl) {
        this.avatarError = t("profilePage.avatarInvalid");
        return;
      }
      await updateAgentIdentity(client, { agentId, avatar: dataUrl });
      await this.context.agentIdentity.ensure([agentId]);
      this.requestUpdate();
    } catch (error) {
      this.avatarError = error instanceof Error ? error.message : String(error);
    } finally {
  ...
  private renderHero(insights: ProfileInsights | null) {
    ...
    const inputId = `profile-avatar-input-${agentId}`;
    return renderSettingsGroup(html`
      <section class="profile-hero">
        <label
          class="profile-hero__avatar profile-hero__avatar--editable"
          for=${inputId}
          title=${t("profilePage.avatarUpload")}
          aria-label=${t("profilePage.avatarUpload")}
        >
          ${this.renderAvatar(avatarUrl, textAvatar, name)}
          <span class="profile-hero__avatar-edit" aria-hidden="true">
            ${this.avatarUploading ? icons.clock : icons.camera}
          </span>
          <input
            id=${inputId}
            class="profile-hero__avatar-file"
            type="file"
            accept="image/*"
            ?disabled=${this.avatarUploading}
            @change=${(event: Event) => void this.handleAvatarPick(agentId, event)}
          />
        </label>
        ${this.avatarError
          ? html`<div class="profile-hero__avatar-error">${this.avatarError}</div>`
          : nothing}
        <div class="profile-hero__name">${name}</div>
        <div class="profile-hero__handle">
          <span>@${agentId}</span>
          <span class="profile-hero__badge">HYClaw</span>
        </div>
